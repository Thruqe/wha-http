...
func Register(w http.ResponseWriter, r *http.Request) {
	...
	existing, _ := db.GetUserByEmail(body.Email)
	if existing != nil {
	...
	user := db.User{ID: uuid.NewString(), Email: body.Email, PasswordHash: hash}
	if err := db.CreateUser(user); err != nil {
		jsonErr(w, err.Error(), 500)
		return
	}
	token, _ := auth.SignJwt(auth.JwtPayload{UserID: user.ID, Email: user.Email})
...
func Login(w http.ResponseWriter, r *http.Request) {
	...
	user, _ := db.GetUserByEmail(body.Email)
