package routes

import (
	"encoding/json"
	"net/http"

	"github.com/google/uuid"
	"github.com/zevlion/wha-http/internal/auth"
	"github.com/zevlion/wha-http/internal/cli"
	"github.com/zevlion/wha-http/internal/db"
)

func ListAccounts(w http.ResponseWriter, r *http.Request) {
	p, err := auth.Authenticate(r)
	if err != nil {
		jsonErr(w, "unauthorized", 401)
		return
	}
	accounts, err := db.GetAccountsByUser(p.UserID)
	if err != nil {
		jsonErr(w, err.Error(), 500)
		return
	}
	if accounts == nil {
		accounts = []db.WaAccount{}
	}
	jsonOK(w, accounts, 200)
}

func AddAccount(w http.ResponseWriter, r *http.Request) {
	p, err := auth.Authenticate(r)
	if err != nil {
		jsonErr(w, "unauthorized", 401)
		return
	}
	var body struct {
		Phone     string `json:"phone"`
		Mode      string `json:"mode"`
		PairPhone string `json:"pairPhone"`
	}
	if err := json.NewDecoder(r.Body).Decode(&body); err != nil {
		jsonErr(w, "invalid JSON", 400)
		return
	}
	if body.Phone == "" {
		jsonErr(w, "phone is required", 400)
		return
	}
	if body.Mode == "" {
		body.Mode = "qr"
	}
	if body.Mode == "pair" && body.PairPhone == "" {
		jsonErr(w, "pairPhone is required for pair mode", 400)
		return
	}
	running, _ := cli.ZevBotIsRunning(body.Phone)
	if running {
		jsonErr(w, "Account already running", 409)
		return
	}
	port, err := db.AllocatePort()
	if err != nil {
		jsonErr(w, err.Error(), 500)
		return
	}
	status := "pending_qr"
	if body.Mode == "pair" {
		status = "pending_pair"
	}
	account := db.WaAccount{
		ID: uuid.NewString(), UserID: p.UserID,
		Phone: body.Phone, Port: port, Status: status,
	}
	if err := db.CreateAccount(account); err != nil {
		jsonErr(w, err.Error(), 500)
		return
	}
	if body.Mode == "pair" {
		err = cli.ZevBotStartWithPairCode(body.Phone, port, body.PairPhone, false)
	} else {
		err = cli.ZevBotStartWithQr(body.Phone, port, false)
	}
	if err != nil {
		jsonErr(w, err.Error(), 500)
		return
	}
	jsonOK(w, map[string]any{"account": account}, 201)
}

func GetAccount(w http.ResponseWriter, r *http.Request, accountID string) {
	p, err := auth.Authenticate(r)
	if err != nil {
		jsonErr(w, "unauthorized", 401)
		return
	}
	account, _ := db.GetAccountByIDAndUser(accountID, p.UserID)
	if account == nil {
		jsonErr(w, "Account not found", 404)
		return
	}
	proc, _ := cli.ZevBotGet(account.Phone)
	jsonOK(w, map[string]any{"account": account, "process": proc}, 200)
}

func RemoveAccount(w http.ResponseWriter, r *http.Request, accountID string) {
	p, err := auth.Authenticate(r)
	if err != nil {
		jsonErr(w, "unauthorized", 401)
		return
	}
	account, _ := db.GetAccountByIDAndUser(accountID, p.UserID)
	if account == nil {
		jsonErr(w, "Account not found", 404)
		return
	}
	if err := cli.ZevBotLogout(account.Phone); err != nil {
		jsonErr(w, err.Error(), 500)
		return
	}
	db.DeleteAccount(accountID)
	jsonOK(w, map[string]any{"ok": true}, 200)
}

func StopAccount(w http.ResponseWriter, r *http.Request, accountID string) {
	p, err := auth.Authenticate(r)
	if err != nil {
		jsonErr(w, "unauthorized", 401)
		return
	}
	account, _ := db.GetAccountByIDAndUser(accountID, p.UserID)
	if account == nil {
		jsonErr(w, "Account not found", 404)
		return
	}
	cli.ZevBotStop(account.Phone)
	db.UpdateAccountStatus(accountID, "disconnected")
	jsonOK(w, map[string]any{"ok": true}, 200)
}

func RestartAccount(w http.ResponseWriter, r *http.Request, accountID string) {
	p, err := auth.Authenticate(r)
	if err != nil {
		jsonErr(w, "unauthorized", 401)
		return
	}
	account, _ := db.GetAccountByIDAndUser(accountID, p.UserID)
	if account == nil {
		jsonErr(w, "Account not found", 404)
		return
	}
	cli.ZevBotRestart(account.Phone)
	db.UpdateAccountStatus(accountID, "pending_qr")
	jsonOK(w, map[string]any{"ok": true}, 200)
}