...
func connectUpstream(accountID string, port int) {
	for attempt := 1; attempt <= maxRetries; attempt++ {
		...
		util.Trace("[proxy] upstream attempt %d/%d for account %s", attempt, maxRetries, accountID)
		...
		util.Trace("[proxy] upstream connected for account %s on port %d", accountID, port)
	...
	util.Error("[proxy] gave up connecting upstream for account %s", accountID)
...
func Handler(w http.ResponseWriter, r *http.Request) {
	...
	util.Trace("[proxy] client connected → account %s", accountID)
	...
	defer func() {
		...
		mu.Unlock()
		close(c.send)
		util.Trace("[proxy] client disconnected from account %s", accountID)
	}()
